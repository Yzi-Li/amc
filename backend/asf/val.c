...
int val_get_const(yz_const *self, struct asf_val *result)
{
	...
	result->data.const_id = self->be_data.val.data.i;
	return 0;
}
...
int val_get_expr(struct expr *src, struct asf_val *result)
{
	...
	result->data.reg = asf_reg_get(asf_yz_type2bytes(src->sum_type));
	return 0;
}

int val_get_extracted_val(yz_extract_val *src, struct asf_val *result)
{
	enum ASF_BYTES bytes = asf_yz_type2bytes(&src->elem->result_type);
	result->type = ASF_VAL_REG;
	result->data.reg = asf_reg_get(bytes);
	return 0;
}
...
int val_get_identifier(struct symbol *src, struct asf_val *result)
{
	...
	if (asf_stack_element2mem(src->backend_status, &result->data.mem) == NULL)
...
int val_get_imm(yz_val *src, struct asf_val *result)
{
	...
	if (src->type.type == YZ_NULL) {
		result->data.imm.type = ASF_BYTES_U64;
		result->data.imm.data.iq = 0;
		return 0;
	}
	result->data.imm.type = asf_yz_type2bytes(&src->type);
	result->data.imm.data.iq = src->data.l;
	return 0;
}
...
int val_get_sym(struct symbol *src, struct asf_val *result)
{
	...
	result->type = ASF_VAL_REG;
	result->data.reg = asf_reg_get(asf_yz_type2bytes(&src->result_type));
	if (src->type == SYM_FUNC_ARG) {
		if (src->argc > asf_call_arg_regs_len)
			goto err_too_many_arg;
		result->data.reg += asf_call_arg_regs[src->argc];
	}
	return 0;
err_too_many_arg:
...
int asf_val_get(yz_val *src, struct asf_val *result)
{
	...
	case AMC_EXPR: return val_get_expr(src->data.v, result); break;
	case AMC_SYM:  return val_get_sym(src->data.v, result);  break;
	case AMC_EXTRACT_VAL:
		return val_get_extracted_val(src->data.v, result);
		break;
	case YZ_CONST:     return val_get_const(src->data.v, result); break;
...
int asf_val_is_unsigned(struct asf_val *val)
{
	switch (val->type) {
	case ASF_VAL_IMM:
		if (REGION_INT(val->data.imm.type, ASF_BYTES_U8, ASF_BYTES_U64))
			return 1;
		return 0;
		break;
	case ASF_VAL_MEM:
